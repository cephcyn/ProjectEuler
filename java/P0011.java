...
public class P0011 {
    ...
    public static List<Integer> products = new LinkedList<Integer>();
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[i].length; j++) {
                int productHorizont = 1;
                int productVertical = 1;
                int productLRDiagonal = 1;
                int productRLDiagonal = 1;
                for (int k = 0; k < NUM_COMPONENTS; k++) {
                    if ((j + k) < grid[i].length) {
                        productHorizont *= grid[i][j + k];
                    } else {
                        productHorizont = 0;
                    }
                    if ((i + k) < grid.length) {
                    ...
                    if ((i + k) < grid.length && (j + k) < grid[i].length) {
                        productLRDiagonal *= grid[i + k][j + k];
                    } else {
                        productLRDiagonal = 0;
                    }
                    if ((i - k) >= 0 && (j + k) < grid[i].length) {
                        productRLDiagonal *= grid[i - k][j + k];
                    } else {
                        productRLDiagonal = 0;
                    }
                    System.out.print((i - k) + " " + (j + k) + ", ");
                }
                System.out.println(productRLDiagonal);
                products.add(productHorizont);
                products.add(productVertical);
                products.add(productLRDiagonal);
                products.add(productRLDiagonal);
            }
        }
        int max = 0;
    ...
    public static void initializeGrid() {
        Scanner wholeGrid = new Scanner(GRID_TEXT);
        for (int i = 0; wholeGrid.hasNextLine(); i++) {
            Scanner line = new Scanner(wholeGrid.nextLine());
            for (int j = 0; line.hasNext(); j++) {
